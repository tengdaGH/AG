...
| IELTS | British English | Laomedeia (F), Alnilam (M), Umbriel (M), Laomedeia (F) |
...
After **every** successful audio generation, you **MUST** write one entry to:

```
/Users/tengda/Antigravity/toefl-2026/audio/audio_voice_log.jsonl
```
...
Use the utility at `.agent/skills/toefl_voice_direction/log_audio.py`:

```bash
cd /Users/tengda/Antigravity/toefl-2026
python .agent/skills/toefl_voice_direction/log_audio.py \
  --item_id "abc123" \
...
import sys
sys.path.append(str(Path(__file__).parent.parent.parent))  # reach toefl-2026 root
from _agent.skills.toefl_voice_direction.log_audio import append_voice_log
...
- [ ] **Convert PCM → WAV/MP3** and save to `frontend/public/audio/listening/`.
