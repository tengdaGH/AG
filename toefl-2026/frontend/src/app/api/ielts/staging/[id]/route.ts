...
export async function GET(
    ...
    try {
        ...
        // Security: ensure file is within staging dir
        if (!path.resolve(filePath).startsWith(path.resolve(STAGING_DIR))) {
        ...
        // Normalize JSON shape into the format the workspace page expects
        ...
        const normalized = {
            ...
            // Include diagnostic data for the dashboard
            _meta: {
                processed_at: raw.processed_at,
                total_questions: raw.questions?.parsed_total_questions ?? 0,
                question_types: [...new Set(questionGroups.map((g: any) => g.type))],
            },
        };

        return NextResponse.json(normalized);
    } catch (error) {
        console.error('Error fetching staging IELTS item:', error);
