...
export const ListenConversation: React.FC<ListenConversationProps> = ({
    audioUrl,
    questionText,
    options,
    speakerImageUrl,
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', width: '100%', height: '100%', backgroundColor: '#FFFFFF', border: '1px solid #767676', fontFamily: 'Times New Roman, Times, serif' }}>
            ...
            {/* Top Thick Black Line (No textual instruction header for Conversation tasks) */}
            ...
            <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
                ...
                <div style={{ flex: '1 1 50%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '40px', gap: '20px' }}>
                    {speakerImageUrl && (
                        <img
                            src={speakerImageUrl}
                            alt="Speakers talking"
                            style={{ maxWidth: '100%', maxHeight: '300px', objectFit: 'contain' }}
                        />
                    )}
                    <audio ref={audioRef} src={audioUrl} controls style={{ width: '80%', maxWidth: '400px' }} />
                    <p style={{ color: '#767676', fontSize: '14px', margin: 0 }}>Listen to the conversation, then answer the question.</p>
                </div>
