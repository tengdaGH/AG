...
# Default log path — caller can override via LOG_DIR env var or set_log_path()
# When used from toefl-2026, the log goes to toefl-2026/audio/audio_voice_log.jsonl
# This is a globally available skill, so the log path must be configurable per project.
_LOG_PATH: Path | None = None


def _resolve_log_path() -> Path:
    """Resolve the log file path. Priority: 1) explicit set_log_path, 2) LOG_DIR env, 3) cwd-based."""
    global _LOG_PATH
    if _LOG_PATH is not None:
        return _LOG_PATH
    log_dir = os.getenv("VOICE_LOG_DIR")
    if log_dir:
        return Path(log_dir) / "audio_voice_log.jsonl"
    # Default: write to audio/ under the current working directory
    return Path.cwd() / "audio" / "audio_voice_log.jsonl"


def set_log_path(path: str | Path):
    """Override the default log path. Call this before append_voice_log()."""
    global _LOG_PATH
    _LOG_PATH = Path(path)
...
def append_voice_log(
    ...
    print(f"[voice_log] Appended entry for {item_id} → {log_path}")
    return log_path
